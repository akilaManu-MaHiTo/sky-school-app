...
import { createOldStudents } from "../../../../api/oldStudentsApi";
...
export default function StudentPromotion() {
  ...
  const { mutate: promoteToOldStudents, isPending: isPromoting } = useMutation({
    mutationFn: createOldStudents,
    onSuccess: () => {
      enqueueSnackbar("Students promoted to Old Student Portal successfully", {
        variant: "success",
      });
  ...
  const handlePromoteSelected = () => {
    ...
    const userIds = studentPromotionData
      .filter((student: any) => selectedStudentIds.includes(student.id))
      .map((student: any) => student?.student?.studentId)
      .filter((id: number | undefined) => id != null);

    if (!userIds.length) {
      enqueueSnackbar("Selected students do not have valid IDs", {
        variant: "warning",
      });
      return;
    }

    promoteToOldStudents(userIds);
  };
